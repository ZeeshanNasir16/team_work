...
module.exports = function checkPermission(action) {
  return async (req, res, next) => {
    console.log('action is ===================== ', action);
    console.log('user type ID is ===================== ', req.userTypeId);
    ...
    const resp = new Promise(async (resolve, reject) => {
      try {
        ...
        const result = await query(req.userTypeId, action);
...
async function query(userTypeId, action) {
  //console.log(req)
  // console.log(routeID);
  // console.log(req.body);
  let res = await QueryDB(
    `select * from roles_permission rp join permissions p on rp.permission_id = p.id
     where rp.user_type_id= ? and p.permission_name=?`,
    [userTypeId, action]
  );
  console.log('res', res);
  return res;
